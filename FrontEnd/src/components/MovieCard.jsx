...
const MovieCard = ({
  ...
  useEffect(() => {
    function handleInFav() {
      setInFav(favIds.includes(id.toString()));
      setIsWatched(watchedIds.includes(id.toString()));
    }

    handleInFav();
  }, [favIds, watchedIds, id]);
  ...
  return (
    <div className="movie-card">
      <NavLink className="btn" to={`/movie/${id}`}>
        <div className="poster-wrapper relative">
          ...
          />
          {isWatched && <div className="overlay absolute top-0 text-2xl ">Watched!</div>}
        </div>
      </NavLink>
      <div className="mt-4">
        <h3>{title}</h3>
      ...
      <div className="details-con flex justify-between align-middle">
        ...
        <div className="btns-con">
          <p className={cls} onClick={HandleFav}>
            <MdFavorite className="w-10 text-3xl" />
          </p>
        </div>
      </div>
      <Toaster />
