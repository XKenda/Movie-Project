    ...
    return (
        <div className="header absolute px-12 py-2 w-full z-20">
            <div className="header-container flex justify-between">
                <NavLink to={'/'} className="header-title text-white sm:text-[20px] md:text-3xl ">Movie Pattreaon</NavLink>
                {
                    user?
                    <div className="user-con w-60 flex justify-around">
                        <NavLink to={`/profile`} key={user?.id} className="user-name sm:text-[20px] md:text-3xl text-white">{user?.firstName}</NavLink>
